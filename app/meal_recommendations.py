                ...
                if alternatives:
                    for i, alt in enumerate(alternatives):
                        with st.expander(f"Alternative {i+1}: {alt.get('high_carb_dish', 'High-Carb Dish')} → {alt.get('healthy_alternative', 'Healthy Alternative')}"):
                            st.write(f"**Instead of:** {alt.get('high_carb_dish', 'High-Carb Dish')}")
                            st.write(f"**Try:** {alt.get('healthy_alternative', 'Healthy Alternative')}")
                            st.write(f"**Benefit:** {alt.get('benefit', 'Better for blood sugar management')}")
                else:
...
def display_meal(meal, index, meal_type, show_feedback=True):
    ...
    # Add color to glycemic load text based on value
    gl_display = glycemic_load
    if isinstance(glycemic_load, str):
        gl_display = glycemic_load.upper()

    # Display meal card - using st.container and columns for better layout
    ...
    with meal_container:
        ...
        col1, col2 = st.columns(2)
        with col1:
            st.markdown(f"**Carbs:** {carbs}")
        with col2:
            st.markdown(f"**Glycemic Load:** {gl_display}")
